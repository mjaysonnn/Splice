...
DESCRIPTION
===========
1. Find user-annotated pragma
2. Run transformation
3. Upload Deployment and Make Lambda Code
...
class ModuleConfigClass:
    ...
    module_dir: str = "import_modules"  # we assume we have import modules installed
    ...
    bucket_for_hybrid_code: str = "<>>"
    bucket_for_lambda_handler_zip: str = "<>"
...
def find_user_annotation_in_code(whole_info_class: WholeASTInfoClass,):
    ...
    logger.info("Find User-Annotated pragma")
...
def set_config_for_each_function(
    ...
    # Lambda, VM, Both
    func_service = provide_user_with_service_type(each_func_name)
    ...
    if func_service.strip():
        ...
        if func_service in ("{IaaS, FaaS}", "FaaS"):
            ...
            provide_user_with_lambda_config(each_func_name, function_info)

            # Scaling Policy Metrics
            provide_user_with_cpu_utilization(each_func_name, function_info)
            provide_user_with_arrival_rate(each_func_name, function_info)
            provide_user_with_memory_utilization(each_func_name, function_info)
...
def set_lambda_config_for_whole_app():
    ...
    lambda_timeout = input(f"lambda_config " f": [time_out (100):\n")

    if lambda_timeout.strip():
        lambda_config_for_whole_app.timeout = lambda_timeout

    lambda_runtime = input(f"lambda_config : " f"[runtime (py37):\n")

    if lambda_runtime.strip():
        lambda_config_for_whole_app.runtime = lambda_runtime

    lambda_name = input(f"lambda_config : " f"[name (None):\n")
...
def provide_user_with_memory_utilization(each_func_name, function_info):
...
def provide_user_with_arrival_rate(each_func_name, function_info):
...
def provide_user_with_cpu_utilization(each_func_name, function_info):
...
def provide_user_with_lambda_config(each_func_name, function_info):
    # Memory
    lambda_config_memory = input(
        f"{each_func_name} lambda_config " f": [memory_size (512):\n"
    )
    ...
    # timeout
    lambda_config_timeout = input(
        f"{each_func_name} lambda_config " f": [time_out (100):\n"
    )
    ...
    # Lambda Runtime
    lambda_config_runtime = input(
        f"{each_func_name} lambda_config : " f"[runtime (py37):\n"
    )
...
def put_pragma_in_functions(whole_ast_info: WholeASTInfoClass):
    """
        ...
        body=[
            ast.Expr(
                value=ast.Call(
                    ...
                    args=[
                        ast.Str(s="bucket-for-splice-compiler"),
...
def function_call_orchestrator(whole_ast_info: WholeASTInfoClass) -> None:
    ...
    if whole_ast_info.offloading_whole_application:
        ...
        sorted_func_info_list = sorted(func_info_list, key=lambda x: x.line_number)
        var = [x.func_name for x in sorted_func_info_list]
    ...
    for each_input_param in input_param_list:
        download_obj = ast.Expr(
            value=ast.Call(
                ...
                args=[
                    ast.Str(s="input-output-splice-bucket"),
                    ast.Name(id=each_input_param, ctx=ast.Load()),
                    ast.BinOp(
                        left=ast.Str(s="/tmp/"),
                        op=ast.Add(),
    ...
    if whole_ast_info.offloading_whole_application:
        ...
        for _, each_compiler_generated_lambda in lambda_function_info.items():
            if isinstance(each_compiler_generated_lambda, CompilerGeneratedLambda):
                ...
                var = each_compiler_generated_lambda.lambda_event_input_objs
...
def make_lambda_code_directory(whole_ast_info: WholeASTInfoClass) -> None:
    ...
    if whole_ast_info.offloading_whole_application:
        ...
        # Fetch module
        var = whole_ast_info.lambda_handler_module_dict
...
def map_func_to_func_arn(whole_ast_info: WholeASTInfoClass) -> None:
    ...
    if whole_ast_info.offloading_whole_application:
        ...
        var = whole_ast_info.lambda_function_info
        ...
        var = whole_ast_info.non_function_object_info
...
def write_hybrid_code(whole_ast_info: WholeASTInfoClass):
    ...
    for each_func_call in lambda_based_func_call_info_list:
        ...
        # Use transformer to replace assign or expr object with newly made object
...
def make_lambda_function_using_aws_cli(whole_ast_info: WholeASTInfoClass):
    ...
    if whole_ast_info.offloading_whole_application:
        ...
        lambda_arn = "arn:aws:lambda:us-east-1:<>:function:" + lambda_name
        ...
        for lambda_name, lambda_info in lambda_info_dict.items():
            ...
            if isinstance(lambda_info, MergedCompilerGeneratedLambda):
                ...
                lambda_name_to_make = "<>" + lambda_name_for_creation
                lambda_zip_name = lambda_info.lambda_group_name + ".zip"
                lambda_arn = (
                    "arn:aws:lambda:us-east-1:<>:function:" + lambda_name_to_make
                )
        ...
        if lambda_handler_module_dict:
            ...
            for (
                ...
                lambda_name_to_make = "<>" + original_file_name
                lambda_zip_name = lambda_handler_func_name + ".zip"
                ".".join([lambda_handler_func_name, lambda_handler_func_name])
                lambda_arn = (
                    "arn:aws:lambda:us-east-1:<>:function:" + lambda_name_to_make
                )
...
def upload_upload_hybrid_code_to_s3hybrid_code_to_s3():
...
def upload_lambda_deployment_to_s3(whole_ast_info: WholeASTInfoClass):
    ...
    for each_deploy_zip in deploy_dir_list:
        upload_deploy_zip_to_s3(each_deploy_zip, whole_ast_info)
...
def main():
    logger.info(f"Directory is {BENCHMARK_DIR} and File is {FILE_NAME}")
